                              ...
                              struct icalrecurrencetype *recur, icaltimetype *t)
{
    ...
    *t = icaltime_null_time();
    ...
    if (month) {
        error |= !icalrecur_set_first_by(&recur->by[ICAL_BY_DAY], nth_weekday(week, (day % 7) + 1));
        error |= !icalrecur_set_first_by(&recur->by[ICAL_BY_MONTH], month);
        ...
        error |= !icalrecur_set_first_by(&recur->by[ICAL_BY_YEAR_DAY], day - 1000);
    } else {
        /* Convert day-of-non-leap-year into month/day */
        icaltimetype t = icaltime_from_day_of_year(day, 1 /* non-leap */);

        error |= !icalrecur_set_first_by(&recur->by[ICAL_BY_MONTH], t.month);
        error |= !icalrecur_set_first_by(&recur->by[ICAL_BY_MONTH_DAY], t.day);
    }
...
icalcomponent *icaltzutil_fetch_timezone(const char *location)
{
    ...
    for (i = 0; i < num_trans; i++) {
        ...
        if (zone->rrule_comp) {
            ...
            // Check if most of the recurrence pattern is the same
                ...
                if ((zone->recur->by[ICAL_BY_DAY].size >= 1) && (by_day == zone->recur->by[ICAL_BY_DAY].data[0])) {
                    ...
                    if (((zone->recur->by[ICAL_BY_MONTH_DAY].size >= 1) && (icaltime.day >= zone->recur->by[ICAL_BY_MONTH_DAY].data[0] + 7))
                        ...
                        if (!icalrecur_set_first_by(&zone->recur->by[ICAL_BY_DAY], nth_weekday(0, dow))) {
                            icalerror_set_errno(ICAL_NEWFAILED_ERROR);
                            break;
                        }
                    }
        ...
        if (!zone->rrule_comp) {
            ...
            error |= !icalrecur_set_first_by(&zone->recur->by[ICAL_BY_DAY], by_day);
            error |= !icalrecur_set_first_by(&zone->recur->by[ICAL_BY_MONTH], icaltime.month);
            error |= !icalrecur_set_first_by(&zone->recur->by[ICAL_BY_MONTH_DAY], icaltime.day);
            if (error) {
                icalerror_set_errno(ICAL_NEWFAILED_ERROR);
                break;
            }
